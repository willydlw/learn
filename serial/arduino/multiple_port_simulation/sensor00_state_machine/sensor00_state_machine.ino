...
*            Transmits hello message every WAIT_ACK_DELAY seconds
*            in case the original hello message was not received
*            by the other connected device.
*            
*            When ACK is received, next state is SEND_SENSOR_ID
*
*   SEND_SENSOR_ID: waits in this state untial a RDY response if
*           received. Tranmits sensor id message every WAIT_READY_DELAY
*           seconds in case the original message was not received
*           by the other connected device.
*           
*           When RDY is received, next state is SENSE.
*
*   SENSE: Purpose is to simulate data a sensor might transmit.
...
// timing 
const unsigned long WAIT_ACK_DELAY = 3000;      // milliseconds
...
void setup(){
  ...
  sensor.sensor_id = sensorIdList[0];
  strcpy(sensor.sensor_name, sensorNameList[0]);
...
void process_received_message(){
  ...
  if(inString.compareTo(ackMessage) == 0){
    if(operateState == OperationalState::INITIAL){
      operateState = OperationalState::SEND_SENSOR_ID;
      
    }
  }
  else if(inString.compareTo(readyMessage) == 0){
    if(operateState == OperationalState::SEND_SENSOR_ID){
...
void loop(){
  ...
  if(messageReceived){
    ...
    // reset message state to await beginning of
    // nexe message received
    rxmsgState = ReceiveMessageState::AWAIT_RECEIPT;
    messageReceived = false;
  }
  ...
  switch(operateState){
    ...
    /* INITIAL state's purpose is to wait for ACK 
     *  signal from serial connected program
     */
    case OperationalState::INITIAL:
      
      // stays here until ACK is received
      if( (millis() - startTime) > WAIT_ACK_DELAY){
        Serial.write(helloMessage);
        startTime = millis();
      }
     break;

    case OperationalState::SEND_SENSOR_ID:
